...
| **TDA pipeline** (v1.0.3+) | `tda_pipeline` — `TDAPipeline` immutable builder; `.rips()/.cech()/.reduce()/.pairs()/.landscape()/.entropy()/.bottleneck()/.wasserstein()`. `from_points` is currently broken and `.reduce()` rejects `'auto'` — see [Known defects](#known-defects) |
...
| **Standard triangulations** (v1.0.4+) | `simplicial_filtration` — `rp2_filtration` (the 6-vertex minimal ℝP² triangulation: V=6, E=15, F=10), `torus_filtration` and `klein_bottle_filtration` (9-vertex grid triangulations: V=9, E=27, F=18 — correct, but not minimal) |
...
| **External bridges** (v1.8.0+, optional) | `experimental.gap_bridge` (group order, structure description, SmallGroups id), `experimental.regina_bridge` (normal surfaces, census recognition), `experimental.snappy_bridge` (hyperbolic volume, census identification). Each shells out to Docker, reports itself unavailable rather than raising, and doubles as an independent oracle — pytop itself stays dependency-free |
...
A comprehensive 16-chapter user guide (`docs/user_guide/`) covering point-set topology and
metric spaces in **four parallel formats**: Python scripts, Jupyter notebooks, Markdown, and
LaTeX (PDF). Every chapter is enriched with intuition + counter-example boxes, theorem
proof sketches, **verified** runnable pytop examples, exercises with worked solutions, and
**46 figures** — the four formats stay in sync via `docs/user_guide/tools/build_chapter_formats.py`.
...
**The bridge family is complete**, and with it every open item on the roadmap.

pytop is pure Python with no runtime dependencies, and stays that way. Where an
established system does something pytop has no route to, the answer is to
interoperate rather than reimplement — Part IV of the roadmap argues for exactly
this. Three optional bridges now do it, sharing one transport and one safety
posture:

| Bridge | Supplies | Independent check on |
|---|---|---|
| `gap_bridge` | group order, structure description, SmallGroups id | SNF abelianization |
| `regina_bridge` | normal surfaces, census recognition | lens-space `H₁` |
| `snappy_bridge` | hyperbolic volume, census identification | Dehn-surgery `H₁` |

```python
from pytop.experimental import SnappySession, snappy_manifold_info

with SnappySession() as s:
    snappy_manifold_info("4_1", session=s).volume     # 2.029883...
```

Each reports itself unavailable rather than raising when Docker or its image is
absent, validates inputs before interpolating anything into generated code,
detects success by a sentinel rather than an exit code, and reaps the
in-container process on timeout — a client-side timeout kills only the
`docker exec` client, which left processes at 100% CPU until it was fixed.

Three stale roadmap entries were corrected in the process: the in-process SnapPy
bridge is no longer "deferred" (it shipped, as a container bridge), Regina-scale
normal surfaces are now reachable, and sheaf cohomology and persistent K-theory
were removed from "deferred (long-range)" — both shipped in v1.4.0.

## What's New in v1.9.0

**Phase 12 is complete (6/6).** Three milestones, and the two that were listed as
blocked or intractable turned out to be neither.

**P12.3 — Homeomorphism** (`pytop.experimental.homeomorphism`). The roadmap
listed "homeomorphism heuristics ... undecidable in general". There is a decidable
subclass worth having: for a **finite** space the topology and its specialization
preorder determine each other, so a bijection is a homeomorphism exactly when it
is a preorder isomorphism. `finite_homeomorphic` decides it and hands back the
bijection; its absence is a proof, not a failed search. Verified against
brute-force enumeration of every bijection on 100 space pairs.

For everything else, `homeomorphism_obstruction` certifies **non**-homeomorphism
from a differing invariant. There is deliberately no verdict meaning "probably
homeomorphic" — agreeing invariants prove nothing.

```python
from pytop.experimental import finite_homeomorphic
from pytop.experimental.spaces import FiniteSpace

s = FiniteSpace("sierpinski", (0, 1), [(), (0,), (0, 1)])
t = FiniteSpace("relabelled", ("a", "b"), [(), ("b",), ("a", "b")])
bool(finite_homeomorphic(s, t))        # True, with the bijection as witness
```

**P12.4 / P12.6 — GAP and Regina bridges.** Both optional; pytop stays
dependency-free and each reports itself unavailable rather than raising.

GAP (inside the SageMath image) names group presentations `van_kampen` leaves
unnamed and acts as an independent oracle for pytop's SNF abelianization. Regina
— whose image is **built** from Debian, since it publishes none and is not on
PyPI — is an independent source of lens-space homology and exposes normal-surface
enumeration, which pure-Python pytop deliberately does not attempt.

```python
from pytop.experimental import ReginaSession, regina_lens_homology

with ReginaSession() as s:
    regina_lens_homology(8, 3, session=s).torsion      # (8,)
```

Both bridges validate their inputs before interpolating anything, detect success
by a sentinel rather than an exit code, and reap the in-container process on
timeout — a client-side timeout kills only the `docker exec` client.
...
on every input (universal cross-validation test; 12,000 tests pass + GUDHI parity
...
registry, with a **Candidates / soft-deprecations** section). API
naming/consistency audit in `docs/API_DESIGN.md` — finding #3 resolved by
soft-deprecating `persistent_homology_optimized` in favour of
`persistent_homology(method="auto")` (note only, no warning pre-1.7.0).
`CONTRIBUTING.md` + GitHub issue/PR templates, and `docs/GOOD_FIRST_ISSUES.md`
(12 curated, file-grounded starter tasks for the `good first issue` label).
...
Complete user guide and API reference (16 chapters, 226 modules, 36+ examples).
...
- **Research-grade computable-space protocol** (`experimental.spaces`) — Phase 1 complete (S1–S5):
  ...
  regular/normal, compact/connected, Lindelöf/separable, first/second-countable), **19 representations**
...
- **Phase 17** — "Rips n=500 in <1 s" holds at `max_dimension=1` (0.996 s) but not at
  `max_dimension=2, max_scale=1.0`, where it takes 9.06 s (950 914 simplices: 2.24 s build + 6.82 s
  reduce). "Memory linear in simplex count" was never measured.
- **Phase 19.1** — "zero ambiguous error messages": roughly 7 of the ~208 `raise ValueError`/`TypeError`
  sites in `src/pytop/*.py` follow the WHY-HOW-THEN pattern.
- **Phase 20.3** — "10+ external contributors, <48 h issue response": the repository has **1 human
  contributor** and 0 open issues; all 12 curated good-first-issues (#35–#46) were closed by the
  maintainer.
