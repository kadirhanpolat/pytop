...
class ProductMetricSpace(Space):
    """Product of two metric spaces with the sup metric.

    d((x₁,y₁), (x₂,y₂)) = max(d_X(x₁,x₂), d_Y(y₁,y₂)).

    All metric-space separation properties lift to the product; compactness,
    connectedness, separability, and second-countability are individually
    determined by the factor spaces.

    Example — the rational plane ℚ²::

        ProductMetricSpace(
            "Q²",
            distance_x=lambda a, b: abs(Fraction(a) - Fraction(b)),
            member_x=lambda p: isinstance(p, (int, Fraction)),
            distance_y=lambda a, b: abs(Fraction(a) - Fraction(b)),
            member_y=lambda p: isinstance(p, (int, Fraction)),
        )
    """
    ...
    def __init__(
        self,
        name: str,
        distance_x: Callable[[Any, Any], Fraction],
        member_x: Callable[[Any], bool],
        distance_y: Callable[[Any, Any], Fraction],
        member_y: Callable[[Any], bool],
        carrier_kind: CarrierKind = CarrierKind.COUNTABLE,
    ) -> None:
        self.name = name
        self.carrier_kind = carrier_kind
        self._distance_x = distance_x
        self._member_x = member_x
        self._distance_y = distance_y
        self._member_y = member_y

    def contains(self, point: Any) -> bool:
        return (
            isinstance(point, tuple)
            and len(point) == 2
            and bool(self._member_x(point[0]))
            and bool(self._member_y(point[1]))
        )

    def point_separation(self, x: Any, y: Any) -> Verdict:
        d = max(
            Fraction(self._distance_x(x[0], y[0])),
            Fraction(self._distance_y(x[1], y[1])),
        )
        radius = d / 2
        return Verdict.true(
            reason=f"open sup-metric balls of radius {radius} around {x!r} and {y!r} are disjoint",
            witness=((x, radius), (y, radius)),
        )
    ...
    def certificate(self, prop: str) -> Verdict | None:
        if prop in {"T0", "T1", "T2", "regular", "normal"}:
            return Verdict.true(
                reason=f"product of metric spaces is metrizable, hence {prop}"
            )
        if prop == "first_countable":
            return Verdict.true(
                reason="product of two metric spaces is first-countable (sup-metric balls of radius 1/n)"
            )
        if prop in {"tychonoff", "T5", "T6"}:
            return Verdict.true(
                reason=f"product of metric spaces is metrizable, hence perfectly normal → {prop}"
            )
        return None
...
class LexicographicSquareSpace(Space):
    """The unit square [0,1]² with the lexicographic order topology.

    The lex order: (a,b) <_lex (c,d)  iff  a < c,  or  (a = c and b < d).
    The order topology is generated by open intervals in this linear order.

    Key mathematical facts:
    - compact and connected (closed bounded interval in a linear order)
    - T5 (linearly ordered space is hereditarily normal)
    - first-countable (rational endpoints give a countable local base)
    - Lindelöf (compact → Lindelöf)
    - NOT second-countable: the vertical fibers {a}×(0,1) form an uncountable
      family of pairwise-disjoint non-empty open sets
    - NOT separable: any dense set must meet each fiber, so density ≥ 𝔠
    - NOT metrizable: compact T2 + metrizable ⟹ second-countable (Urysohn),
      but it is not second-countable

    Points are represented as 2-tuples of :class:`~fractions.Fraction` (or int)
    values in [0, 1].
    """

    _CERTS: dict[str, bool] = {
        "T0": True, "T1": True, "T2": True, "regular": True, "normal": True,
        "tychonoff": True, "T5": True,
        "compact": True, "connected": True, "lindelof": True,
        "first_countable": True,
        "second_countable": False, "separable": False,
    }

    _REASONS: dict[str, str] = {
        "T0": "linear order is antisymmetric, so distinct points have distinct order neighbourhoods",
        "T1": "in a linear order, every singleton is closed (intersection of two rays)",
        "T2": "linear order topology is Hausdorff (disjoint open rays separate any two points)",
        "regular": "linearly ordered space is T3 (monotonically normal ⟹ regular)",
        "normal": "linearly ordered space is T4 (monotonically normal)",
        "tychonoff": "linearly ordered space is completely regular (T3.5)",
        "T5": "linearly ordered space is hereditarily normal (T5)",
        "compact": "closed bounded interval in the lex order is compact by the order-compactness theorem",
        "connected": "linearly ordered space with no cuts is connected",
        "lindelof": "compact implies Lindelöf",
        "first_countable": "open intervals with rational endpoints form a countable local base at each point",
        "second_countable": (
            "the vertical fibers {a}×(0,1) for a ∈ (0,1) form an uncountable family "
            "of pairwise-disjoint non-empty open sets"
        ),
        "separable": (
            "any dense set must meet each fiber {a}×(0,1) for every a ∈ (0,1); "
            "uncountably many such fibers ⟹ no countable dense set"
        ),
    }

    def __init__(self, name: str = "lex_square([0,1]²)") -> None:
        self.name = name
        self.carrier_kind = CarrierKind.UNCOUNTABLE
    ...
    def contains(self, point: Any) -> bool:
        if not (isinstance(point, tuple) and len(point) == 2):
            return False
        try:
            fa, fb = Fraction(point[0]), Fraction(point[1])
        except (TypeError, ValueError):
            return False
        return Fraction(0) <= fa <= Fraction(1) and Fraction(0) <= fb <= Fraction(1)

    def point_separation(self, x: Any, y: Any) -> Verdict:
        """Separate two distinct lex-square points by an open-ray split."""
        a1, b1 = Fraction(x[0]), Fraction(x[1])
        a2, b2 = Fraction(y[0]), Fraction(y[1])
        if a1 != a2:
            lo_a, hi_a = (a1, a2) if a1 < a2 else (a2, a1)
            split = (lo_a + hi_a) / 2
            lower_pt, upper_pt = (x, y) if a1 < a2 else (y, x)
            return Verdict.true(
                reason=(
                    f"lex split at first coord {split}: "
                    f"{lower_pt!r} in initial segment, {upper_pt!r} in terminal segment"
                ),
                witness={"split_first_coord": split},
            )
        # Same first coordinate — split within the fiber by second coordinate.
        lo_b, hi_b = (b1, b2) if b1 < b2 else (b2, b1)
        split_b = (lo_b + hi_b) / 2
        lower_pt, upper_pt = (x, y) if b1 < b2 else (y, x)
        return Verdict.true(
            reason=(
                f"lex split at second coord {split_b} inside fiber a={a1}: "
                f"{lower_pt!r} below, {upper_pt!r} above"
            ),
            witness={"fiber_first_coord": a1, "split_second_coord": split_b},
        )
    ...
    def certificate(self, prop: str) -> Verdict | None:
        ...
        reason = self._REASONS.get(prop, f"lex square is {'' if val else 'not '}{prop}")
        if val:
            return Verdict.true(reason=reason)
        _counterexamples: dict[str, Any] = {
            "second_countable": "{a}×(0,1) for uncountably many a forms an uncountable disjoint family",
            "separable": "meeting each {a}×(0,1) requires a point at each of uncountably many a-values",
        }
        return Verdict.false(reason=reason, counterexample=_counterexamples.get(prop))

    def cardinal_certificate(self, invariant: str) -> CardinalValue | None:
        # character = ℵ₀  (first-countable via rational-endpoint local base)
        # cellularity = 𝔠  (fibers {a}×(0,1) are pairwise-disjoint open sets, |a| = 𝔠)
        return {
            "character": CardinalValue.aleph_0(),
...
class CantorSpaceRepresentation(Space):
    """The Cantor space {0,1}^ω with the product topology.

    Points are represented as finite binary tuples — the first n bits of the
    infinite binary sequence identifying the point. Two points with differing
    bit at position k are separated by the clopen cylinder
    C_k(v) = {z : z[k] = v}.

    Key mathematical facts:
    - compact (Tychonoff product of compact {0,1})
    - metrizable and T6 (countable product of metric spaces)
    - totally disconnected (clopen cylinders disconnect any two points)
    - perfect (every point is a limit point)
    - second-countable and separable (finite binary strings are a countable
      dense subset; cylindric base is countable)

    Brouwer's theorem: every compact metrizable zero-dimensional perfect
    space is homeomorphic to the Cantor space.
    """
    ...
    _CERTS: dict[str, bool] = {
        ...
        "connected": False,
        "lindelof": True,
        "separable": True, "second_countable": True, "first_countable": True,
    }

    _REASONS: dict[str, str] = {
        "T0": "metrizable spaces are T0",
        "T1": "metrizable spaces are T1",
        "T2": "metrizable spaces are Hausdorff (T2)",
        "regular": "metrizable spaces are regular (T3)",
        "normal": "metrizable spaces are normal (T4)",
        "tychonoff": "metrizable spaces are completely regular (T3.5)",
        "T5": "metrizable spaces are completely normal (T5)",
        "T6": "metrizable spaces are perfectly normal (T6)",
        "compact": "Tychonoff product of the compact space {0,1} is compact",
        "connected": "Cantor space is totally disconnected: clopen cylinders separate any two points",
        "lindelof": "compact implies Lindelöf",
        "separable": "finite binary strings form a countable dense subset of {0,1}^ω",
        "second_countable": "countable product of second-countable spaces is second-countable",
        "first_countable": "second-countable implies first-countable",
    }

    def __init__(self, name: str = "Cantor space {0,1}^ω") -> None:
        self.name = name
        self.carrier_kind = CarrierKind.UNCOUNTABLE

    def contains(self, point: Any) -> bool:
        return isinstance(point, tuple) and all(b in (0, 1) for b in point)

    def point_separation(self, x: Any, y: Any) -> Verdict:
        """Separate by the clopen cylinder at the first differing bit.

        Returns ``undecidable`` when one tuple is a prefix of the other —
        the separation requires knowledge of the full infinite sequence.
        """
        ...
        for k in range(min_len):
            if x[k] != y[k]:
                ...
                return Verdict.true(
                    reason=(
                        f"clopen cylinder C_{k}({v}) = {{z : z[{k}] = {v}}} "
                        f"contains {x!r} but not {y!r}"
                    ),
                    witness={"bit_position": k, "value": v},
                )
        return Verdict.undecidable(
            f"one of {x!r}, {y!r} is a prefix of the other; "
            "separation requires the full infinite sequence"
        )
    ...
    def certificate(self, prop: str) -> Verdict | None:
        if prop not in self._CERTS:
            return None
        val = self._CERTS[prop]
        reason = self._REASONS.get(prop, f"Cantor space is {'' if val else 'not '}{prop}")
        if val:
            return Verdict.true(reason=reason)
        _counterexamples: dict[str, Any] = {
            "connected": "the clopen cylinder {z : z[0] = 0} disconnects the space into two non-empty parts",
        }
        return Verdict.false(reason=reason, counterexample=_counterexamples.get(prop))

    def cardinal_certificate(self, invariant: str) -> CardinalValue | None:
        # weight = ℵ₀ (second-countable: cylinder base is countable)
        # density = ℵ₀ (separable; no finite dense subset — infinite convergent sequences)
        # character = ℵ₀ (first-countable via cylinder-shrinking local base)
        # cellularity = ℵ₀ (separable ⟹ c(X) ≤ ℵ₀; cylinders witness ℵ₀)
        return {
            "weight": CardinalValue.aleph_0(),
            "density": CardinalValue.aleph_0(),
            "character": CardinalValue.aleph_0(),
...
def rational_plane(name: str = "Q²") -> ProductMetricSpace:
    """The rational plane ℚ² with the product sup metric."""

    return ProductMetricSpace(
        name,
        distance_x=lambda a, b: abs(Fraction(a) - Fraction(b)),
        member_x=lambda p: isinstance(p, (int, Fraction)),
        distance_y=lambda a, b: abs(Fraction(a) - Fraction(b)),
...
def cantor_space(name: str = "Cantor space {0,1}^ω") -> CantorSpaceRepresentation:
    """The Cantor space: compact, metrizable, totally disconnected, perfect."""

    return CantorSpaceRepresentation(name)


def lexicographic_square(name: str = "lex_square([0,1]²)") -> LexicographicSquareSpace:
    """The unit square [0,1]² with the lexicographic order topology."""

    return LexicographicSquareSpace(name)
