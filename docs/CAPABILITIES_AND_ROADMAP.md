...
| **P12.6** | Regina integration | ⬜ | Split out of P12.4 on evidence: no public Regina image (`regina/regina`, `regina-normal/regina`, `benburton/regina` all absent), not in the Sage image, no PyPI distribution. Needs a purpose-built image mirroring `pytop-snappy`. |
...
| Phase 12 milestones complete | 4 / 6 ✅⬜ (sheaf cohomology, persistent K-theory, countably infinite complexes, GAP bridge done; homeomorphism heuristics and Regina open — Regina was split out as P12.6 on evidence that no usable image exists) |
...
| **Current version** | **v1.7.0** — live on PyPI as **`pytopology`** (`import pytop`) |
