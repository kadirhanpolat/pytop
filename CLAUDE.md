...
- **Released v0.9.6 (latest):** first "frontier" closed (PR #23) — `is_planar` now uses the `O(V+E)` **left-right planarity test** (Brandes 2009) instead of the exponential rotation-system search, so it decides any graph and **never raises** (`W9…W40`, large grids that used to raise `GraphPlanarityError` now return `True`). `graph_genus` unchanged. Validated against networkx on **all** ≤6-vertex graphs (33 867, 0 disagreements) + random larger. 9 960 tests pass. Remaining frontier: dual/cohomology persistence (lower value — pytop defers large TDA to GUDHI).
