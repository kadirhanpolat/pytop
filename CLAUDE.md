...
3-manifolds, degree theory, cardinal functions, and more. As of **v1.10.0** it ships a
...
- **Optional external bridges** (`experimental.{gap,regina,snappy}_bridge`) — a fourth layer for work
  pytop deliberately does not do itself. Part IV of the roadmap argues for interoperating rather than
  reimplementing, and these are that: GAP names group presentations `van_kampen` leaves unnamed,
  Regina enumerates normal surfaces, SnapPy computes hyperbolic volume. Each also doubles as an
  **independent oracle** for something pytop computes its own way (SNF abelianization, lens-space
  homology, Dehn-surgery homology). **pytop stays dependency-free**: every bridge shells out to
  Docker, and `*_available()` returns `False` rather than raising. They share one transport (warm
  container, fresh interpreter per call) and one safety posture — inputs validated before anything is
  interpolated into generated code, success detected by a sentinel rather than an exit code, and the
  in-container process reaped on timeout. Do not add a bridge that skips any of those three.
...
- **v1.8.0 / v1.9.0 / v1.10.0** ✅ (2026-09-23): Phase 12 completed, then the bridge family
- **v2.0.0** (Q2 2027): break with the 18-month deprecation window; drop 3.11 support
