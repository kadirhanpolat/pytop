...
- **Released v1.6.0 (Phase 16 P16.1–P16.3 Framework Complete):** **Empirical validation & oracle ecosystem** — P16.1 benchmark suite ✅ (37 tests: minimal triangulations, knot tables, large graphs, performance baselines); P16.2 oracle parity framework ✅ (extended knot table 6→40 primes, SnapPy H₁ Dehn surgery, K-theory AHSS validation, 8 oracle tests); P16.3 statistical validation ✅ (10K random ER 1-skeleta, pytop H₀/H₁ 100% success, avg 6.26ms/complex, JSON report, outlier analysis). `tests/validation/`: 79 tests passing (benchmark+oracle+statistical). **Knot table:** 40 primes (unknot–9_5); next: expand to 50+. **Oracle integration:** framework ready, GUDHI/Ripser Betti computation pending. **Status:** All Phase 16 framework milestones (1–3) foundation-complete; oracle parity matrix ready for expansion. Validation suite production-ready for pytop-only baseline; oracle cross-checks awaiting proper API integration.
...
| Milestone | Module | Description |
|-----------|--------|-------------|
| **P16.1** ✅ | `benchmark_suite` | Public dataset collection: minimal triangulations (T²/Klein/ℝP² with verified Betti/torsion), 40-knot table (unknot–9_5 with Alexander/Jones), large grid library (3×3–40×40, all confirmed planar). 37 tests; ruff/mypy clean. |
| **P16.2** ✅ | `oracle_parity` | Oracle parity framework: extended knot table (6→40 primes), SnapPy H₁ Dehn surgery oracle, K-theory rational AHSS validation (S¹, S²). **Complete:** OracleAgreement, AgreementMatrix, 40-prime knot table, SnapPy tests, K-theory validation, 8 tests. Ready for 50+ knot expansion + full oracle matrix. |
| **P16.3** ✅ | `statistical_validation` | Random complex validation: 10K random Erdős–Rényi 1-skeleta (5–50 vertices), pytop H₀/H₁ computation (100% success), outlier analysis, JSON report. Avg 6.26ms/complex. Framework complete; oracle integration (GUDHI/Ripser) pending. |
...
- P16.2 ✅ framework complete (OracleAgreement/AgreementMatrix, extended knot table 40 primes, SnapPy H₁ oracle, K-theory AHSS validation; 8 oracle tests, 79 total validation tests). Next: expand to 50+ knots + populate full oracle agreement matrix.
- P16.3 ✅ framework complete + 10K run complete (10,000 complexes, 100% pytop success, avg 6.26ms/complex)
