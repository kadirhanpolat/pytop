...
| **P20.3** ✅ | `community_onboarding` | `CONTRIBUTING.md` (dev setup, two-layer architecture, code style, API rules, tests, deprecations, conventional commits + PR flow, issue reporting, first-timer pointer). GitHub issue templates: `bug_report.yml`, `feature_request.yml`, `docs.yml` + `config.yml` (+ `PULL_REQUEST_TEMPLATE.md`). `good first issue` label exists on GitHub; 12 curated, file-grounded starter tasks staged in `docs/GOOD_FIRST_ISSUES.md` for the maintainer to open as labelled issues. Commit: 714b5c6 (+ this branch). | **Complete** (docs/templates/backlog). Opening the 10+ GitHub issues + 48h SLA are live-repo ops left to the maintainer. |
